...
new #[Layout('layouts.admin')] class extends Component 
{
    ...
    {
        // 1. Загружаем альбомы с фото и жалобы
        $this->user = $user->load([
        ...
        // 3. Берем адрес из БД, если его нет — запрашиваем по координатам
...
<div x-data='{
    ...
    <div class="bg-card border border-border rounded-lg p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="p-4 bg-muted/20 rounded-lg border border-border">
                            ...
                            <p class="text-sm font-medium mt-1 flex items-center gap-1">
                                @if ($user->gender === 'male')
                                    <x-lucide-mars class="w-4 h-4 text-blue-500" /> Мужской
                                @elseif($user->gender === 'female')
                                    <x-lucide-venus class="w-4 h-4 text-pink-500" /> Женский
                                @else
                ...
                <div class="space-y-4">
                    ...
                    <div class="p-4 bg-muted/20 rounded-lg border border-border">
                        ...
                        <p class="text-sm mt-1 text-muted-foreground">
                            {{ $user->about ?? 'Пользователь не заполнил информацию о себе' }}</p>
                    </div>
                            ...
                            @if ($user->interests && is_array($user->interests) && count($user->interests) > 0)
                                @foreach ($user->interests as $interest)
                                    <x-ui.badge variant="secondary" size="xs"
                                        wire:key="interest-{{ $loop->index }}">{{ $interest }}</x-ui.badge>
                                @endforeach
                            @else
        ...
        <div x-show="tab === 'photos'" style="display: none;">
            <!--  Оберточный div убивает глюк Livewire Morphing с дублированием альбомов -->
            <div>
                    ...
                    @foreach ($user->albums as $album)
                        <div class="mb-8 last:mb-0" wire:key="album-{{ $album->id }}">
                            <h3 class="text-lg font-medium mb-3 flex items-center gap-2">
                                ...
                                @endif
                                <span class="text-sm text-muted-foreground font-normal">({{ $album->photos->count() }}
                                    фото)</span>
                            </h3>
                                ...
                                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                                    @foreach ($album->photos as $photo)
                                        <div wire:key="photo-{{ $photo->id }}"
                                            class="relative group border border-border rounded-lg overflow-hidden bg-muted">
                                            ...
                                            <!-- Действия при наведении (pointer-events-none пропускает клик к картинке) -->
                                            <div
                                                class="absolute right-4 bottom-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <div class="flex gap-2 pointer-events-auto">
                                                    @if (!$photo->is_primary)
                                                        <x-ui.button wire:click="setPrimaryPhoto({{ $photo->id }})"
                                                            wire:confirm="Сделать это фото основным?"
                                                    ...
                                                    @endif
                                                    <x-ui.button wire:click="deletePhoto({{ $photo->id }})"
                                                        wire:confirm="Удалить это фото навсегда?"
                                                        variant="destructive" size="icon-sm"
                                                        class="text-white hover:bg-destructive/60" title="Удалить">
        ...
        <div x-show="tab === 'sessions'" style="display: none;" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="flex items-center gap-4 p-4 bg-muted/30 rounded-lg border border-border">
                    <div class="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary">
                        <x-lucide-clock class="w-5 h-5" /></div>
                    <div>
                ...
                <div class="flex items-center gap-4 p-4 bg-muted/30 rounded-lg border border-border">
                    <div class="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary">
                        <x-lucide-wifi class="w-5 h-5" /></div>
                    <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="flex items-center gap-4 p-4 bg-muted/30 rounded-lg border border-border">
                    <div class="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary">
                        <x-lucide-calendar class="w-5 h-5" /></div>
                    <div>
                ...
                <div class="flex items-center gap-4 p-4 bg-muted/30 rounded-lg border border-border">
                    <div class="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary">
                        <x-lucide-edit class="w-5 h-5" /></div>
                    <div>
        ...
        <div x-show="tab === 'moderation'" style="display: none;" class="space-y-4">
            ...
            <div class="p-4 bg-muted/30 rounded-lg border border-border">
                ...
                @php
                    ...
                    $receivedVariant =
                        $receivedCount == 0 ? 'success' : ($receivedCount <= 5 ? 'warning' : 'destructive');
                    $sentVariant = $sentCount == 0 ? 'success' : ($sentCount <= 5 ? 'warning' : 'destructive');
                @endphp

                <div class="space-y-2">
                    <p class="text-sm text-muted-foreground flex items-center gap-2">
                        Жалоб на пользователя: <x-ui.badge :variant="$receivedVariant"
                            size="sm">{{ $receivedCount }}</x-ui.badge>
                    </p>
                    <p class="text-sm text-muted-foreground flex items-center gap-2">
                        Жалоб от пользователя: <x-ui.badge :variant="$sentVariant"
                            size="sm">{{ $sentCount }}</x-ui.badge>
                    </p>
                </div>
            </div>
    ...
    <!-- Вкладка 5: КАРТА (не трогал инициализацию) -->
    <!-- Инициализируем Alpine с памятью вкладок и надежным вотчером для карты -->
                ...
                <div class="space-y-4">
                    ...
                    <div class="p-4 bg-muted/20 rounded-lg border border-border">
                        ...
                        <div class="mt-2 space-y-2">
                            <x-ui.input wire:model="editLat" label="Широта" type="number" step="any"
                                oninput="updateMarkerFromInputs()" />
                            <x-ui.input wire:model="editLng" label="Долгота" type="number" step="any"
                                oninput="updateMarkerFromInputs()" />
                    ...
                    <div class="p-4 bg-muted/20 rounded-lg border border-border">
                        <p class="text-xs text-muted-foreground">🔁 Перетащите маркер, чтобы изменить позицию, или
...
@push('scripts')
<script>
    // Отключаем Fancybox hash, чтобы не ломал URL
    ...
    window.setupMap = function(initialLat, initialLng, initialAddress, user) {
        ...
        // 1. ГЛАВНЫЙ ФИКС: Если карта уже создана, просто обновляем её размер.
        // Это решает проблему x-show и скрытых контейнеров.
        if (window.leafletMap) {
            setTimeout(() => {
                window.leafletMap.invalidateSize();
                if (window.leafletMarker) {
                    window.leafletMarker.openPopup();
                }
            }, 50); // Небольшая задержка, чтобы браузер успел отрисовать display: block
            return;
        }

        // 2. Инициализация (выполняется только один раз)
        ...
        window.leafletMarker.on('dragend', async function(e) {
            ...
            // Вызываем метод Livewire для получения адреса
        ...
        // 4. Функция обновления из инпутов (вызывается через oninput)
        ...
        // Финальный фикс размера после полной отрисовки
        setTimeout(() => {
