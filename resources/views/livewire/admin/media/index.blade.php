...
new #[Layout('layouts.admin')] class extends Component 
{
    ...
    public function mediaItems()
    {
        ...
        /**
     * Показать тост об успешном копировании.
     */
    public function notifyCopied(string $key = ''): void
    {
        $message = $key ? "Путь скопирован: {$key}" : 'URL скопирован';
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                        @foreach($this->mediaItems as $media)
                            ...
                            <div wire:key="media-{{ $media->id }}" 
                                 @contextmenu.prevent="activeMedia = { id: {{ $media->id }}, url: '{{ $media->getVariantUrl('lg') }}' }"
                                 class="relative group aspect-square rounded-lg overflow-hidden border border-border bg-muted/10">
                               
                              
                                <a href="{{ $media->getVariantUrl('lg') }}" data-fancybox="media-gallery" data-caption="{{ $media->file_name }}" class="absolute inset-0 w-full h-full block z-10">
                                    {{-- В сетку грузим 'sm' (или дефолтный путь, если sm нет) --}}
